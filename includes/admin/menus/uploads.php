...
final class NF_FU_Admin_Menus_Uploads extends NF_Abstracts_Submenu {
	...
	protected function get_tabs() {
		$tabs = array(
			'browse'   => __( 'Browser Uploads', 'ninja-forms-uploads' ),
	...
	public function display() {
		...
		NF_File_Uploads()->template( 'admin-menu-uploads.html', compact( 'tabs', 'active_tab', 'save_button_text', 'table' ) );
	}
	...
	protected function update_settings() {
		...
		foreach ( $whitelist as $key => $setting ) {
			...
			NF_File_Uploads()->controllers->settings->set_setting( $key, sanitize_text_field( $value ) );
			$updates = true;
		}
